...
--The resulting table should include the following columns: Employee Number, First Name and Last Name (using LPAD or RPAD for formatting), 
--Age, and Seniority

SELECT EMPLOYEE_NUMBER, 
    LEFT(LAST_NAME + REPLICATE(' ', 18), 18) AS Last_Name_RPAD,
    LEFT(FIRST_NAME + REPLICATE(' ', 18), 18) AS First_Name_RPAD,
    DATEDIFF(DAY, BIRTH_DATE, GETDATE()) AS Age,
    DATEDIFF(DAY, HIRE_DATE, GETDATE()) AS Seniority
FROM EMPLOYEES
WHERE TITLE NOT IN ('Mrs', 'Miss') AND IFNULL(SALARY, 0) + IFNULL(COMMISSION, 0) >= 8000
GROUP BY EMPLOYEE_NUMBER, LAST_NAME, FIRST_NAME, BIRTH_DATE, HIRE_DATE
ORDER BY Seniority DESC;



--Display products that meet the following criteria: (C1) quantity is packaged in bottle(s), (C2) 
--the third character in the product name is 't' or 'T', (C3) supplied by suppliers 1, 2, or 3, (C4) unit price ranges between 70 and 200,
--and (C5) units ordered are specified (not null). The resulting table 
--should include the following columns: product number, product name, supplier number, units ordered, and unit price.

SELECT PRODUCT_REF, PRODUCT_NAME, SUPPLIER_NUMBER, UNITS_ON_ORDER AS Units_Ordered, UNIT_PRICE
FROM PRODUCTS
...
WHERE EXISTS (SELECT * FROM SUPPLIERS 
              WHERE SUPPLIERS.SUPPLIER_NUMBER = 1
...
AND (CATEGORIES.CATEGORY_NAME = 'Desert' OR CATEGORIES.CATEGORY_NAME IS NULL) 
GROUP BY CUSTOMERS.CUSTOMER_CODE
HAVING COUNT(CASE WHEN CATEGORIES.CATEGORY_NAME = 'Desserts' THEN 1 ELSE NULL END) <= 1;
...
SELECT CUSTOMERS.CUSTOMER_CODE, CUSTOMERS.COMPANY, CUSTOMERS.PHONE, CUSTOMERS.COUNTRY,
    ISNULL(SUM(ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY * (1 - ORDER_DETAILS.DISCOUNT)), 0) AS TOTAL_AMOUNT
FROM CUSTOMERS 
LEFT JOIN ORDERS 
    ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS 
...
WHERE CUSTOMERS.COUNTRY = 'France'
  AND ( ORDERS.ORDER_DATE IS NULL OR
        (  YEAR(ORDERS.ORDER_DATE) = 1998 AND 
            MONTH(ORDERS.ORDER_DATE) = 4 AND 
            DATENAME(WEEKDAY, ORDERS.ORDER_DATE) = 'Monday'))
GROUP BY CUSTOMERS.CUSTOMER_CODE, CUSTOMERS.COMPANY, CUSTOMERS.PHONE, CUSTOMERS.COUNTRY;
...
SELECT CUSTOMERS.CUSTOMER_CODE, CUSTOMERS.COMPANY AS COMPANY_NAME, CUSTOMERS.PHONE AS TELEPHONE_NUMBER
FROM CUSTOMERS 
JOIN ORDERS 
ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
JOIN ORDER_DETAILS 
ON ORDERS.ORDER_NUMBER = ORDER_DETAILS.ORDER_NUMBER
GROUP BY CUSTOMERS.CUSTOMER_CODE, CUSTOMERS.COMPANY, CUSTOMERS.PHONE
HAVING COUNT(DISTINCT ORDER_DETAILS.PRODUCT_REF) = (SELECT COUNT(DISTINCT PRODUCT_REF) FROM PRODUCTS);
...
SELECT CUSTOMERS.CUSTOMER_CODE, COUNT(ORDER_DETAILS.QUANTITY) AS N0_of_Orders
FROM CUSTOMERS
 LEFT JOIN ORDERS 
ON CUSTOMERS.CUSTOMER_CODE = ORDERS.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS 
...
SELECT 
    COUNT(CASE WHEN YEAR(ORDERS.ORDER_DATE) = 1996 THEN 1 END) AS ORDERS_IN_1996,
    COUNT(CASE WHEN YEAR(ORDERS.ORDER_DATE) = 1997 THEN 1 END) AS ORDERS_IN_1997,
    COUNT(CASE WHEN YEAR(ORDERS.ORDER_DATE) = 1997 THEN 1 END) - 
    COUNT(CASE WHEN YEAR(ORDERS.ORDER_DATE) = 1996 THEN 1 END) AS DIFFERENCE
FROM ORDERS 
WHERE YEAR(ORDERS.ORDER_DATE) IN (1996, 1997);
